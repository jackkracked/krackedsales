...
export function CallDetailModal({ call, onClose }: { call: Call | null; onClose: () => void }) {
  ...
  return createPortal(
    <div data-r10n-modal-overlay className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6">
      <div data-r10n-call-backdrop className="absolute inset-0 bg-ink/55 backdrop-blur-sm" onClick={onClose} />
      ...
      <div data-r10n-modal data-r10n-call-modal className="relative z-10 flex w-full max-w-[1000px] max-h-[92vh] flex-col overflow-hidden rounded-[16px] bg-card shadow-2xl ring-1 ring-border" role="dialog" aria-modal="true">
        ...
        <div data-r10n-modal-header className="flex items-center gap-3 border-b border-border bg-gradient-to-b from-muted/30 to-transparent px-5 py-4 shrink-0">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {insights?.sentimentLabel && (
                <span
                  ...
                  data-sentiment={insights.sentimentLabel}
                  className={cn(
            ...
            <div data-r10n-call-metarow className="flex items-center gap-x-2.5 gap-y-1 flex-wrap mt-1 text-[11px] text-muted-foreground">
              <span data-r10n-call-typebadge data-kind={call!.callType === "dialer" ? "dialer" : "meet"} className={cn("inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full font-semibold", call!.callType === "dialer" ? "bg-slate-100 text-slate-600" : "bg-blue-50 text-blue-700")}>
              ...
              {call!.status && STATUS_META[call!.status] && (
                <span data-r10n-status-pill data-r10n-call-statuspill data-status={call!.status} className={cn("px-1.5 py-0.5 rounded-full font-semibold", STATUS_META[call!.status].cls)}>{STATUS_META[call!.status].label}</span>
              )}
              {call!.direction && (
                <span data-r10n-call-direction data-dir={call!.direction} className="inline-flex items-center gap-1 font-medium">
                  {call!.direction === "outbound" ? <ArrowUp className="w-3 h-3" /> : <ArrowDown className="w-3 h-3" />}
                  {call!.direction === "outbound" ? "Outbound" : "Inbound"}
                </span>
              )}
              {call!.repName && <span data-r10n-call-metachip className="font-medium">{call!.repName}</span>}
              <span data-r10n-call-metachip className="inline-flex items-center gap-1"><Clock className="w-3 h-3" />{fmtDuration(call!.durationSeconds)}</span>
              <span data-r10n-call-metachip>{formatDateTime(call!.startedAt)}</span>
            </div>
          </div>
          <button data-r10n-call-close onClick={onClose} className="p-1.5 -mr-1 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted transition-colors shrink-0" aria-label="Close">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 md:grid-cols-[1.15fr_1fr]">
          ...
          <div data-r10n-call-leftpanel className="min-h-0 overflow-y-auto border-b md:border-b-0 md:border-r border-border p-4 space-y-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              {call!.fathomShareUrl && (
                <a data-r10n-call-link href={call!.fathomShareUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-primary hover:underline"><PlayCircle className="w-3.5 h-3.5" />Watch on Fathom</a>
              )}
              {isUpcoming && call!.meetingUrl && (
                <a data-r10n-call-link href={call!.meetingUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-primary hover:underline"><Video className="w-3.5 h-3.5" />Join Google Meet</a>
              )}
              {call!.smartNotesUrl && (
                <a data-r10n-call-link href={call!.smartNotesUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-primary hover:underline"><StickyNote className="w-3.5 h-3.5" />Smart Notes</a>
              )}
            </div>

            {/* AI summary */}
            {data?.fathomSummary && (
              <div data-r10n-call-summary className="rounded-[10px] border border-border bg-muted/20 overflow-hidden">
                <button onClick={() => setSummaryOpen((v) => !v)} className="w-full flex items-center gap-2 px-3.5 py-2.5 text-left hover:bg-muted/30 transition-colors">
                  <span data-r10n-call-summary-label className="text-[10.5px] font-bold uppercase tracking-widest text-primary shrink-0">AI Summary</span>
            ...
            {cards.length > 0 && (
              <div className="grid grid-cols-2 gap-2">
                {cards.map(({ key, label, Icon, color }) => (
                  <div key={key} data-r10n-call-insight data-insight={key} className="rounded-[8px] border border-border/70 bg-muted/20 px-3 py-2.5 min-w-0">
                    <div className="flex items-center gap-1.5 mb-1.5"><Icon data-r10n-call-insight-icon className={cn("w-3.5 h-3.5 shrink-0", color)} /><span data-r10n-call-insight-label className="text-[10.5px] font-bold uppercase tracking-widest text-muted-foreground">{label}</span></div>
          ...
          <div data-r10n-call-transcript-panel className="flex min-h-0 flex-col">
            <div data-r10n-call-transcript-head className="flex items-center gap-2 px-4 py-2.5 border-b border-border shrink-0">
              <Captions className="w-3.5 h-3.5 text-muted-foreground" />
              <span data-r10n-call-transcript-title className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground">Transcript</span>
              {hasVideo && entries.length > 0 && <span data-r10n-call-clock className="ml-auto text-[10px] tabular-nums text-muted-foreground/60">{fmtClock(currentTime)}</span>}
            </div>
            <div data-r10n-call-transcript-body className="min-h-0 flex-1 overflow-y-auto px-4 py-3">
              {entries.length === 0 ? (
                      ...
                      <div key={i}>
                        {showSpeaker && (
                          <div className="flex items-center gap-1.5 mt-3 mb-1">
                            <span data-r10n-call-speaker-dot data-rep={isRep} className={cn("w-2 h-2 rounded-full shrink-0", speakerColor(e.speaker))} />
                            <span data-r10n-call-speaker-name className="text-[11px] font-semibold text-foreground">{e.speaker}</span>
                            {isRep && <span data-r10n-call-rep-tag className="text-[9px] font-bold uppercase tracking-wide text-primary/70">Rep</span>}
                          </div>
                        )}
                        <button
                        ...
                        >
                          {hasVideo && (
                            <span data-r10n-call-line-ts data-active={isActive} className={cn("shrink-0 mt-0.5 text-[10px] tabular-nums w-9", isActive ? "text-primary" : "text-muted-foreground/40 group-hover:text-primary")}>{fmtClock(e.sec)}</span>
                          )}
