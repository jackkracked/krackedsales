...
function StatPill({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex flex-col items-center px-3 py-2 bg-muted/40 rounded-[8px] min-w-[64px]">
      <span className={cn("text-base font-bold", highlight ? "text-primary" : "text-foreground")}>
        {value}
      </span>
...
function ConditionPill({ condition }: { condition: TemplateCondition }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-muted text-muted-foreground text-[11px] rounded-full border border-border/60">
...
export function TemplateCard({
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {template.isWinner && (
              <span className="flex items-center gap-1 px-2 py-0.5 bg-amber-100 text-amber-700 text-[11px] font-semibold rounded-full">
          ...
          {conditions.length > 0 ? (
            ...
            <p className="text-xs text-muted-foreground mt-1">Matches all leads</p>
          )}
        </div>
      ...
      {/* Body preview */}
      <p className="text-xs text-muted-foreground leading-relaxed line-clamp-2 mb-3 bg-muted/30 rounded-[7px] px-3 py-2 border border-border/60">
        {template.body}
      </p>
