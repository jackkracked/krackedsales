...
function channelBadge(item: QueueItem) {
  if (item.platform === "instagram") return { label: "IG", className: "bg-pink-50 text-pink-600 ring-1 ring-pink-200/60" };
  if (item.platform === "facebook") return { label: "FB", className: "bg-blue-50 text-blue-600 ring-1 ring-blue-200/60" };
  if (item.channel === "TikTok") return { label: "TT", className: "bg-slate-50 text-slate-600 ring-1 ring-slate-200/60" };
...
function QueueListItem({
  ...
  return (
    <button
      ...
      className={cn(
        "relative w-full text-left px-4 py-3 border-b border-border/60 last:border-0 transition-colors",
        isSelected ? "bg-primary/[0.06]" : "hover:bg-muted/30"
      )}
    >
      <div className="flex items-center gap-2 mb-0.5">
        <span data-r10n-convo-channelpill data-r10n-rq-channel={item.channel} className={cn("inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold shrink-0", badge.className)}>
          {badge.label}
        </span>
        <span data-r10n-convo-name className="text-[13px] font-semibold text-foreground truncate flex-1">
          {item.contactName}
        </span>
        <span data-r10n-rq-stale data-fresh={item.staleDays === 0 ? "true" : undefined} data-overdue={item.staleDays > 3 ? "true" : undefined} className={cn("text-xs tabular-nums shrink-0", staleColor(item.staleDays))}>
          {staleLabel(item.staleDays)}
        </span>
      </div>
      <p data-r10n-convo-preview className="text-xs text-muted-foreground truncate pl-[34px]">
...
function ExternalThreadPanel({ item }: { item: QueueItem }) {
  ...
  return (
    <>
        ...
        {rawMessages.map((m, i) => {
          ...
          return (
            <div key={m.id ?? i} className={cn("flex", isOut ? "justify-end" : "justify-start")}>
              <div data-r10n-bubble data-dir={isOut ? "out" : "in"} className={cn(
                ...
                <p>{m.text}</p>
                {m.createdAt && (
                  <p data-r10n-bubble-time className={cn("text-[10px] mt-1", isOut ? "text-primary-foreground/60" : "text-muted-foreground/60")}>
      ...
      {/* Compose */}
      {sent ? (
        <div data-r10n-rq-sent className="shrink-0 border-t border-border px-4 py-3 flex items-center gap-2 text-xs text-emerald-600">
          <CheckCircle2 className="w-3.5 h-3.5" />
          Message sent
        </div>
      ) : (
        <div data-r10n-rq-compose className="shrink-0 border-t border-border px-4 py-3 space-y-2">
          {!draftLoaded && (
            <div data-r10n-rq-draftloading className="flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          <div className="flex items-center justify-between">
            {draftLoaded && (
              <span data-r10n-rq-aidraft className="text-[11px] text-muted-foreground/60 flex items-center gap-1">
...
function ThreadPanel({ item, onBack }: { item: QueueItem; onBack: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Thread header */}
      <div data-r10n-thread-header className="shrink-0 flex items-center gap-2 px-4 py-3 border-b border-border">
        <button
          onClick={onBack}
          data-r10n-thread-action-icon
          className="p-1 rounded text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors lg:hidden"
        >
          <ArrowLeft className="w-3.5 h-3.5" />
        </button>
        <span data-r10n-convo-channelpill data-r10n-rq-channel={item.channel} className={cn("inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold", badge.className)}>
          {badge.label}
        </span>
        <span data-r10n-thread-name className="text-[13px] font-semibold text-foreground truncate">{item.contactName}</span>
      </div>
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <div className={cn(
        ...
        {/* Header */}
        <div data-r10n-list-header className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
          <div className="flex items-center gap-2">
            <Inbox className="w-3.5 h-3.5 text-muted-foreground" />
            <span data-r10n-list-title className="text-[13px] font-semibold text-foreground">Reply Queue</span>
            {data && data.total > 0 && (
              <span data-r10n-rq-count className="min-w-[18px] h-[18px] rounded-full bg-red-500 text-white text-[10px] font-bold flex items-center justify-center px-1">
                {data.total}
              </span>
            )}
          </div>
